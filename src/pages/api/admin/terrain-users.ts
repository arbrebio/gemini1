...
/**
 * Terrain field-staff (engineer/technician) account management, run
 * directly from the main admin portal — there is no separate "Terrain
 * super admin" account. Gated by requireAdminAuth, same as every other
 * /api/admin/* route, so the one super-admin login that manages sales,
 * projects, customers and suppliers also creates/manages terrain field
 * staff, instead of a second admin identity.
 */
...
/**
 * GET /api/admin/terrain-users — list all terrain field-staff accounts.
 * POST /api/admin/terrain-users — create a new account { full_name, email, role, phone? }.
 * PUT /api/admin/terrain-users — { id, action: 'reset-password' | 'toggle-active' } or
 *                                 { id, full_name?, role?, phone? } for a general field update.
 * DELETE /api/admin/terrain-users — { id } — permanently removes the account.
 */
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    console.error('[admin terrain-users GET]', e);
  ...
  try {
    ...
    const { data: profile, error: profileErr } = await supabase
      ...
      .insert({
        ...
        temp_password, created_by: auth.userId,
      })
      .select('id, full_name, email, role, phone, avatar_url, active, must_change_password, created_at')
      .single();
    ...
    console.error('[admin terrain-users POST]', e);
    return json({ error: 'Internal server error' }, 500);
  }
};
...
export const PUT: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    console.error('[admin terrain-users PUT]', e);
    return json({ error: 'Internal server error' }, 500);
  }
};
...
export const DELETE: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    console.error('[admin terrain-users DELETE]', e);
    return json({ error: 'Internal server error' }, 500);
  }
};
